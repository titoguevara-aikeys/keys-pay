...
export default function NiumAdminPage() {
  ...
  const checkHealth = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/nium/health');
  ...
  const createBeneficiary = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/nium/beneficiaries', {
        method: 'POST',
  ...
  const webhookUrl = typeof window !== 'undefined' 
    ? `${window.location.origin}${process.env.NEXT_PUBLIC_NIUM_WEBHOOK_PATH || '/api/nium/webhook'}`
    : '/api/nium/webhook';
    ...
    <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                value={quoteForm.destinationCurrency}
                onChange={(e) => setQuoteForm(prev => ({ ...prev, destinationCurrency: e.target.value }))}
              />
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="payoutCustomerHashId">Customer Hash ID</Label>
              <Input
                id="payoutCustomerHashId"
                value={payoutForm.customerHashId}
                onChange={(e) => setPayoutForm(prev => ({ ...prev, customerHashId: e.target.value }))}
                placeholder="UUID"
              />
            </div>
            <div>
              <Label htmlFor="payoutWalletHashId">Wallet Hash ID</Label>
              <Input
                id="payoutWalletHashId"
                value={payoutForm.walletHashId}
                onChange={(e) => setPayoutForm(prev => ({ ...prev, walletHashId: e.target.value }))}
                placeholder="UUID"
              />
            </div>
            <div>
          ...
          {results.payout && (
            <div className="space-y-2">
              <h4 className="font-medium">Payout Result:</h4>
              <pre className="bg-muted p-4 rounded text-sm overflow-auto">
      ...
      {/* NIUM Sandbox Screenshot */}
      <Card>
        <CardHeader>
          <CardTitle>NIUM Sandbox Integration</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <img 
              src="/nium/nium-sandbox-keys.png" 
              alt="NIUM Sandbox - Keys Pay Integration" 
            ...
            />
            <div className="hidden p-8 bg-muted rounded-lg text-center">
              <h3 className="text-lg font-medium">NIUM Sandbox – Keys Pay</h3>
              <p className="text-muted-foreground">Client ID: a10a45ae-242a-4311-b25f-80adc03be4f6</p>
            </div>
          </div>
        </CardContent>
      </Card>
