-- RLS Policies and Security Fixes for Keys Pay Provider Architecture
...
-- Ledger entries policies (read-only for users, system can insert)
...
CREATE POLICY "Users can create NymCard customer records"
ON public.nymcard_customers FOR INSERT
WITH CHECK (user_id = auth.uid() AND public.is_org_member(auth.uid(), organization_id));

CREATE POLICY "System can update NymCard customers"
ON public.nymcard_customers FOR UPDATE
USING (true);
...
CREATE POLICY "Users can create NymCard cards"
ON public.nymcard_cards FOR INSERT
WITH CHECK (user_id = auth.uid());

CREATE POLICY "System can update NymCard cards"
ON public.nymcard_cards FOR UPDATE
USING (true);
...
-- Provider-specific order policies
CREATE POLICY "Users can view Guardarian orders for their crypto orders"
ON public.guardarian_orders FOR SELECT
USING (EXISTS (
  SELECT 1 FROM public.crypto_orders 
  WHERE id = crypto_order_id AND user_id = auth.uid()
));
...
CREATE POLICY "Users can view Ramp orders for their crypto orders"
...
-- Webhook events policies (system only)
